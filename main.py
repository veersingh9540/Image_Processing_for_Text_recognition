...
def Search():
    ...
    def database_search():
        ...
        SLi= imageSL.subsample(1,2)
         #label for image
        cam_canvasSL= tk.Label(frameS ,width=50,height=50, image=SLi)
        cam_canvasSL.place( relwidth=1,relheight=1)
        Connect.commit()
        Connect.close()
    
    submit_button = tk.Button(search_frame, text='SUBMIT', command =database_search)
...
frame3= tk.Frame(root , bg='#000000',bd=5)
